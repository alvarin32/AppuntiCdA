...
\end{defin}
\begin{esem} \label{intersezdirett}
...
\end{defin}
\begin{prop} \label{condizionidianello}
...
\newline
\emph{Dimostrazione:} $(\Rightarrow)$ Se $\mathfrak{A}$ è un anello, per ogni $A,B \in\mathfrak{A}$ 
...
è una $\sigma$-algebra.\\
\newline
\emph{Dimostrazione:} è un fatto ovvio.
\begin{flushright}
\emph{c.v.d.}
\end{flushright}
\end{prop}
...
\newline
\emph{Dimostrazione:} Si ha $\mathcal{C}_1\subseteq\sigma(\mathcal{C}_2)$ e quindi 
...
\end{displaymath}
\emph{Dimostrazione:} Sia $\mathcal{T}$ la topologia euclidea e $\mathcal{Q}:=\{(a,+\infty)|  
...
\newline
\emph{Dimostrazione:} Sia $\mathcal{T}$ la topologia euclidea e poniamo 
\begin{displaymath}
...
\end{center}
da cui segue la tesi. \begin{flushright} \emph{c.v.d.} \end{flushright}
\end{teo}
...
\newline
\emph{Dimostrazione.}
Per ogni $\varepsilon >0$ sia $C \subseteq A$ elementare e chiuso tale che valga
\begin{equation}
...
\end{multline*}
e la tesi segue dall'arbitrarietà di $\varepsilon$. \begin{flushright} \emph{c.v.d.} \end{flushright}
\end{teo}
\begin{defin}
...
\newline
\emph{Dimostrazione.} Sia $A$ un insieme elementare e $\{A_{n}\}_{n}$ una famiglia numerabile di insiemi
...
\end{displaymath} 
\begin{flushright} \emph{c.v.d.} \end{flushright}
\end{teo}
...
\newline
\emph{Dimostrazione.} Rifacendoci alla definizione \ref{defsubaddit}
...
\newline 
\emph{Dimostrazione.} Sia $A$ un insieme misurabile; allora per definizione esiste $E$ 
...
\end{flushright}
\end{teo}
A questo punto abbiamo descritto in maniera esauriente (perlomeno, per quanto ci serve operativamente) il
primo dei tre concetti fondamentali che ci siamo proposti di trattare. Partendo dai rettangoli, siamo arrivati 
a definire una misura ``universale''; nel capitolo successivo completeremo questo discorso e lo
generalizzeremo, trattando di misure \emph{qualsiasi}.
\end{section}
\end{chapter}
\begin{chapter}{Complementi}
\lettrine[lines=1]{I}{n questo capitolo} ci occuperemo di caratterizzare ulteriormente
(giusto quattro concetti) la misura di Lebesgue; principalmente, però, esporremo
...
\end{itemize}
\emph{Dimostrazione:} $i)$ Siccome $R = A\cup {(R\smallsetminus A)}$ allora la subadditivtà di $\mu^*$ implica 
...
insieme di misura esterna nulla è misurabile (il lettore è vivamente pregato di dimostrarlo per esercizio), segue 
...
\end{itemize}
\emph{Dimostrazione:} $i)$ L'additività di $\mathfrak m$ implica $\mathfrak m(\varnothing)=\mathfrak m(\varnothing\cup\varnothing)=2\mathfrak m
(\varnothing)=0$.\\
...
\newline
\emph{Dimostrazione:} Preso un elemento $B\in\mathfrak{A}(\mathfrak{S})$, allora esistono 
 degli insiemi 
...
\end{itemize}
\emph{Dimostrazione:} $i)$ Dalla proposizione \ref{proprietàmisura} segue che 
...
\newline
\emph{Dimostrazione:} Sia $A\in\mathfrak{A}(\mathfrak{S})$ e sia $\{B_n\}_{n=1}^{\infty}$ una partizione 
...
\end{itemize}
\emph{Dimostrazione:} $i)$ Poniamo $B_n:={(A\cap A_n)}\smallsetminus\bigcup_{k=1}^{n-1}A_k$: ovviamente $B_n\in
...
$\mathfrak m'$.\\
\newline
\emph{Dimostrazione:} Se $\mathfrak m'$ è $\sigma$-additiva, per la proposizione \ref{sigmasemisupadditività} vale $ii)$.\\
...
\end{itemize}
\emph{Dimostrazione.} $i)$ Dalla definizione di $C$ segue che $C=\bigcap_{n=1}^{\infty}C_n$ con i $C_n$ tutti
...
\newline
\emph{Dimostrazione:} $(\Leftarrow)$ Supponiamo che valga la definizione \ref{primadefdimis}: sappiamo che per ogni
...
\newline
\emph{Dimostrazione:} Se $A$ è Peano-Jordan misurabile, allora per ogni $\varepsilon >0$ esistono $A',A''$ 
...
misurabile.\\
\newline 
\emph{Dimostrazione.}  Siccome $g$ è misurabile segue che per ogni $A \in \mathfrak{S}_Z$ allora $f^{-1} (A) 
\in \mathfrak{S}_Y$; inoltre $f$ è misurabile,
e quindi $f^{-1}(g^{-1} (A)) = (g \circ f)^{-1}(A) \in \mathfrak{S}_X$, cioé $g \circ f$ è misurabile.  
\begin{flushright} \emph{c.v.d.}
\end{flushright}
\end{prop}
...
\newline \emph{Dimostrazione.}  $(\Rightarrow)$ Segue subito dalla definizione.\\
...
\newline \emph{Dimostrazione.}  Siccome $f$ è continua, sappiamo che per ogni aperto $A \in \mathfrak{B}(Y)$ segue
...
\newline \emph{Dimostrazione.}  Sia $A\subset X$ l'insieme ove $f(x)=g(x)$: allora su $A$ vale 
\begin{displaymath}
...
\newline \emph{Dimostrazione.}  Sia $\{f_n\}_{n \in \mathbb{N}}$ una successione di funzioni misurabili e poniamo $f := 
...
\newline \emph{Dimostrazione.}  Sia $A\subset X$ l'insieme ove $\lim_{n\to +\infty}f_n(x)=f(x)$. Allora, sull'insieme $A$, risulta che $f$ è una 
...
\newline \emph{Dimostrazione.}  $(\Rightarrow)$ Se, per assurdo, $f\nsim g$, allora esistono un insieme $B\subset X$ con $\mu(B)\neq 0$
...
\end{itemize}
\emph{Dimostrazione.}  Definiamo gli insiemi
\begin{displaymath}
...
\newline \emph{Dimostrazione.}  $(\Rightarrow)$ Sia $f_n$ convergente in misura a $g$ ; 
...
\newline \emph{Dimostrazione.}  Sia $A$ l'insieme ove $f_n\nto f$; definiamo inoltre i seguenti insiemi:
\begin{displaymath}
...
\newline \emph{Dimostrazione.} Sia $\{\varepsilon_k\}_{k\in\mathbb{N}}$ una succesione tale che $\lim_{n\to +\infty}\varepsilon_k = 0$ 
...
\end{displaymath}
\item[$c.$] \color{red} MANCA IL PEZZO DI MISURA INFINITA \color{black}
\end{itemize} 
\begin{oss}
...
\end{displaymath}
\emph{Dimostrazione.} Ovviamente dev'essere $|  f(x)|\leq\phi(x)$, da cui otteniamo subito l'integrabilità della
...
\end{displaymath}
\emph{Dimostrazione.} Siccome le $f_n$ sono limitate, esiste $M>0$ tale che $|  f_n(x)|\leq M$. Allora basta porre $\phi(x)=M$ ed 
...
\lim_{n\to \infty} \int_A f_n(x)d\mu = \int_A f(x)d\mu.
\end{displaymath}
\emph{Dimostrazione.} Anzitutto notiamo che, senza perdere in generalità, possiamo assumere che $f_1(x) \geq 0$;
...
\end{displaymath}
\emph{Dimostrazione.} Poniamo $S_k:=\sum_{n=1}^k f_k(x)$: dall'ipotesi si deduce immediatamente che 
\begin{displaymath}
...
\end{displaymath}
\emph{Dimostrazione.} Poniamo nuovamente $\phi_n := \inf_{k\geq n}f_k(x)$; siccome per ogni $k\geq n$ si ha che $\phi_n\leq f_k$, 
...
\\
\emph{Dimostrazione.} Proviamo che presa una qualunque successione $\{y_n\}_{n\in\mathbb{N}}$ in $(a,b)$ convergente ad un certo $y_0$,
...
\end{displaymath}
\end{itemize}
\emph{Dimostrazione.} Fissato un certo $y_0\in (a,b)$, definiamo la funzione 
\begin{displaymath}
...
Riemann.\newline
Per questioni di semplicità daremo una dimostrazione di questo fatto nel caso in cui $f$
...
\\
\emph{Dimostrazione.} Essendo $A$ misurabile, esiste $C_n=\bigcup_r\Delta_{nr}$ per certi \footnote{Ricordiamo che con $\mathfrak{R}$ indichiamo la famiglia di tutti i rettangoli.}
...
\end{displaymath}
\emph{Dimostrazione.} La dimostrazione si articola in diversi passi:
...
\end{displaymath}
\emph{Dimostrazione.} è una banale applicazione del lemma \ref{misuraprodotto}: in tal caso abbiamo
\begin{displaymath}
...
\end{displaymath}
\emph{Dimostrazione.} Iniziamo a considerare il caso in cui $f(x,y) \geq 0$; nel caso generale basterà notare
